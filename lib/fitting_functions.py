...
def fitting_functions():
    ...
    To add a new template product function:
    1. Create a new .py file in lib/fcns/ with fcn_*, txt_*, dct_* and x_dim
    2. The function will be automatically included - no changes needed here!
    
    Returns:
        dict: A dictionary where keys are input counts (x_dim values) and values 
              are lists of dictionaries containing template product function definitions.
    """
    
    # Dynamically import all modules from fcns directory and organize by x_dim
